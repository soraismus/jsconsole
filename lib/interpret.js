...
function addChar(appState, char) {
  document.getElementsByClassName('jsconsole-prompt-text')[0].innerText += char
}
...
function deleteRightChar() {
  var innerText = document.getElementsByClassName('jsconsole-prompt-text-post-cursor')[0].innerText;
  document.getElementsByClassName('jsconsole-prompt-text-post-cursor')[0].innerText = innerText.slice(1, innerText.length);
}
...
function fastForwardHistory(appState) {
  ...
  console.log('appState.history.past', appState.history.past);
  console.log('appState.history.future', appState.history.future);
  console.log('appState.history.future LAST', appState.history.future[appState.history.future.length - 1]);
  ...
  appState.history.past.push(__text);
  console.log('appState', appState);
  console.log('appState.history.past', appState.history.past);
  console.log('appState.history.future', appState.history.future);
  console.log('----');
...
function rewindHistory(appState) {
  ...
  var __promptTextPost = document.getElementsByClassName('jsconsole-prompt-text-post-cursor')[0].innerText;
  var __text = String.trim(__promptText + __promptTextPost);
  appState.history.future.push(__text);
  console.log('appState', appState);
  console.log('appState.history.past', appState.history.past);
  console.log('appState.history.future', appState.history.future);
  console.log('----');
...
function submit(appState) {
  ...
  appState.history.past.push(String.trim(__text));
...
var appState = { history: { past: [], future: [] }};
...
document.addEventListener(
  ...
  function (event) {
    ...
    switch (event.keyCode) {
      case 13: // enter
        interpreter.submit(appState);
        break;
      case 8: // backspace
        interpreter.deleteLeftChar(appState);
        break;
      case 37: // left;
        event.preventDefault();
        interpreter.moveCursorLeft(appState);
        break;
      case 39: // right;
        event.preventDefault();
        interpreter.moveCursorRight(appState);
        break;
      case 38: // up;
        interpreter.rewindHistory(appState);
        break;
      case 40: // down;
        interpreter.fastForwardHistory(appState);
        break;
      case 46: // delete
        interpreter.deleteRightChar(appState);
        break;
      case 0:
        interpreter.addChar(appState, String.fromCharCode(event.charCode));
        break;
    }
  });
